...
void ClassTable::install_basic_classes() {
  ...
  Class_ Object_class = class_(
      ...
      filename);
  inheritance_graph.set_base_class(Object_class);
  //
  ...
  inheritance_graph.add_child(IO_class->get_parent(), IO_class);
  ...
  inheritance_graph.add_child(Int_class->get_parent(), Int_class);
  ...
  inheritance_graph.add_child(Bool_class->get_parent(), Bool_class);
  ...
  inheritance_graph.add_child(Str_class->get_parent(), Str_class);
}

bool ClassTable::check_illegal_inheritance(Class_ cls) {
  return cls->get_parent()->equal_string(Int->get_string(), Int->get_len()) ||
         cls->get_parent()->equal_string(Bool->get_string(), Bool->get_len()) ||
         cls->get_parent()->equal_string(Str->get_string(), Str->get_len()) ||
         cls->get_parent()->equal_string(SELF_TYPE->get_string(),
                                         SELF_TYPE->get_len());
}
...
void ClassTable::install_user_defined_classes(Classes classes) {
  ...
  for (int i = classes->first(); classes->more(i); i = classes->next(i)) {
    ...
    if (check_illegal_inheritance(cls)) {
      semant_error(cls) << "Class " << cls->get_name()
                        << " cannot inherit class " << cls->get_parent()
                        << ".\n";
      return;
    }

    if (inheritance_graph.exists(cls)) {
      semant_error(cls) << "Class " << cls->get_name()
                        << " was previously defined.\n";
      return;
    }

    if (!inheritance_graph.add_child(cls->get_parent(), cls)) {
      semant_error(cls) << "Class " << cls->get_name()
    ...
    if (cls->get_name()->equal_string(Main->get_string(), Main->get_len())) {
      main_defined = true;
    }
  }
...
void class__class::assign_types(TypeEnvironment typeenv) {
  ...
  typeenv.M->enterscope();
  typeenv.C->enterscope();

  // How to get methods from base classes (Maybe get their environment?)

  typeenv.C->addid(SELF_TYPE, &name);
...
void method_class::assign_types(TypeEnvironment typeenv) {
  typeenv.M->addid(name, &return_type);
...
void formal_class::assign_types(TypeEnvironment typeenv) {
  if (name->equal_string(self->get_string(), self->get_len())) {
    typeenv.classtable->semant_error(
        typeenv.classtable->get_class(*typeenv.C->lookup(SELF_TYPE)))
...
void no_expr_class::assign_types(TypeEnvironment typeenv) { type = No_type; }
...
void string_const_class::assign_types(TypeEnvironment typeenv) { type = Str; }

void bool_const_class::assign_types(TypeEnvironment typeenv) { type = Bool; }

void int_const_class::assign_types(TypeEnvironment typeenv) { type = Int; }
...
void dispatch_class::assign_types(TypeEnvironment typeenv) {
  ...
  for (int i = actual->first(); actual->more(i); i = actual->next(i)) {
    actual->nth(i)->assign_types(typeenv);
  }

  Symbol *def = typeenv.M->lookup(name);
  if (def) {
    type = *def;
  }
}
...
void program_class::semant() {
  ...
  TypeEnvironment env{&O, &M, &C, classtable};
